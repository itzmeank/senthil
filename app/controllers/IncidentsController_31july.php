...
class IncidentsController extends BaseController {
	...
	public function index()
	{
		...
		//} else {
			//$incidents = Incident::where('incident_branch', '=', Auth::user()->branch)->get();
		//}
		...
		for ( $i = 0; $i < count($followups); $i++ ) {
		    ...
		    $followup = new Followup;
				...
				$affected = DB::table('followups')
					...
					//->update(array('first_name' => $witnesses[0]['first_name']));
                  ->update(array('date' => $followups[1]['date'],'staff' => $followups[1]['staff'],'whom' => $followups[1]['whom'],'notes' => $followups[1]['notes']));
		    ...
		    $followup = new Followup;
				...
				$affected = DB::table('followups')
					...
					//->update(array('first_name' => $witnesses[0]['first_name']));
                  ->update(array('date' => $followups[2]['date'],'staff' => $followups[2]['staff'],'whom' => $followups[2]['whom'],'notes' => $followups[2]['notes']));
		    ...
		    $followup = new Followup;
				...
				$affected = DB::table('followups')
					...
					//->update(array('first_name' => $witnesses[0]['first_name']));
                  ->update(array('date' => $followups[3]['date'],'staff' => $followups[3]['staff'],'whom' => $followups[3]['whom'],'notes' => $followups[3]['notes']));
	...
	public function generatemail($id)
	{
	 ...
	 //return 'Hello world';
		...
		$html2pdf->WriteHTML(nl2br($content)); 
