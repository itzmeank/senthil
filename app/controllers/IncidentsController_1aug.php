...
class IncidentsController extends BaseController {
	...
	public function index()
	{
		...
		//} else {
			$incidents = Incident::where('user_id', '=', Auth::user()->id)->get();
			// $incidents = DB::table('incidents')->paginate(10);
		//}
		...
		for ( $i = 0; $i < count($followups); $i++ ) {
			if ( $followups[$i]['staff'] ) {
				...
				$affected = DB::table('followups')
					...
					//->update(array('first_name' => $witnesses[0]['first_name']));
                    ->update(array('date' => $followups[0]['date'],'staff' => $followups[0]['staff'],'whom' => $followups[0]['whom']));
			}		
		}
		    ...
		    $followup = new Followup;
				...
				$affected = DB::table('followups')
					...
					//->update(array('first_name' => $witnesses[0]['first_name']));
                  ->update(array('date' => $followups[1]['date'],'staff' => $followups[1]['staff'],'whom' => $followups[1]['whom']));
		    ...
		    $followup = new Followup;
				...
				$affected = DB::table('followups')
					...
					//->update(array('first_name' => $witnesses[0]['first_name']));
                  ->update(array('date' => $followups[2]['date'],'staff' => $followups[2]['staff'],'whom' => $followups[2]['whom']));
		    ...
		    $followup = new Followup;
				...
				$affected = DB::table('followups')
					...
					//->update(array('first_name' => $witnesses[0]['first_name']));
                  ->update(array('date' => $followups[3]['date'],'staff' => $followups[3]['staff'],'whom' => $followups[3]['whom']));
